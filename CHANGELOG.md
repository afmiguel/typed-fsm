...
### Fixed
- **CI Fixes**: Properly removed `examples/avr_blink.rs` from the repository (it was previously only removed from `Cargo.toml` but remained in the filesystem, causing CI failures).
- **Docs**: Fixed a formatting error in `src/lib.rs` that triggered clippy warnings.

## [0.4.7] - 2025-12-02

### Fixed
- **Removed `examples/avr_blink.rs`**: This example required Nightly Rust and caused CI failures on Stable. The Arduino example is now fully hosted in the [external repository](https://github.com/afmiguel/typed-fsm-arduino-test) linked in the documentation.
